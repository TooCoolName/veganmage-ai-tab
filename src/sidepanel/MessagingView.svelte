<script lang="ts">
  ...
  const filteredMessages = $derived(messages.filter((m) => m.tabId === selectedTab?.id).slice(-2));
  ...
  export const loadActiveTabs = async () => {
      try {
          ...
          const tabs = allTabs.filter((tab) => {
              ...
              const providerObj = providersList.find((p: any) => p.id === provider);
  ...
  async function sendMessageToTab() {
      ...
      try {
          ...
          showStatus(`Failed: ${error.message || 'Error'}`, 'error');
      } finally {
          isSending = false;
      }
  }
...
<div class="flex flex-col h-full gap-4">
  ...
  <div class="rounded-xl border bg-card text-card-foreground shadow shrink-0">
      <div class="flex flex-col p-4">
          <div class="flex justify-between items-center mb-3">
              <h3 class="font-semibold leading-none tracking-tight">Active Tabs ({activeTabs.length})</h3>
              <button onclick={loadActiveTabs} class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-xs font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 disabled:pointer-events-none hover:text-accent-foreground underline">
                  Refresh
              </button>
          </div>
              ...
              <div class="space-y-1 max-h-32 overflow-y-auto pr-1">
                  {#each activeTabs as tab (tab.id)}
                      ...
                      <div
                          ...
                          class="flex items-center gap-2 p-2 rounded cursor-pointer transition-all border {selectedTab?.id === tab.id ? 'bg-primary text-primary-foreground border-primary shadow-sm' : 'bg-background hover:bg-muted border-transparent'}"
                      >
                          <div class="flex-1 min-w-0">
                              <div class="font-medium text-sm truncate">{tab.title}</div>
                              <div class="text-[10px] opacity-70 truncate">{tab.providerName}</div>
                          </div>
                          {#if tab.active}
                              <div class="inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors bg-transparent {selectedTab?.id === tab.id ? 'text-primary-foreground border-primary-foreground/30' : 'text-foreground'}">Active</div>
                          {/if}
                      </div>
                  {/each}
              </div>
  ...
  {#if selectedTab}
      <div class="rounded-xl border bg-card text-card-foreground shadow overflow-hidden flex-1 flex flex-col min-h-0">
          <div class="flex flex-col h-full min-h-0">
              
              <!-- Messages -->
              <div bind:this={scrollRef} class="flex-1 overflow-y-auto p-4 space-y-4 bg-muted/30 scroll-smooth min-h-0">
                  {#if filteredMessages.length === 0}
                      <div class="h-full flex items-center justify-center text-muted-foreground italic text-sm">
                  ...
                  {:else}
                      {#each filteredMessages as msg (msg.id)}
                          <div class="flex flex-col {msg.role === 'user' ? 'items-end' : 'items-start'}">
                              ...
                              <div class="text-sm px-4 py-2 rounded-2xl max-w-[85%] break-words {msg.role === 'user' ? 'bg-primary text-primary-foreground rounded-tr-sm' : 'bg-muted text-foreground rounded-tl-sm'}">
                                  {msg.text}
                              </div>
                          </div>
                      {/each}
                  ...
                  {#if isSending}
                      <div class="flex flex-col items-start">
                          <div class="opacity-50 text-[10px] mb-1">{selectedTab.providerName}</div>
                          <div class="text-sm px-4 py-2 rounded-2xl bg-muted text-foreground rounded-tl-sm w-16 flex justify-center opacity-70">
  ...
  {#if !selectedTab}
      <div class="rounded-xl border bg-card text-card-foreground shadow">
          <div class="p-8 flex items-center justify-center text-center text-muted-foreground">
              <p class="text-sm">Select a tab from the list above to start messaging</p>
          </div>
      </div>
  {/if}
</div>
